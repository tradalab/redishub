...
export const FileUpload = ({ label, hint, multiple = false, hasError, formats, onUploaded }: FileUploadProps) => {
  ...
  return (
    <div>
      ...
      >
        ...
        {!!hint && <Label className="text-ui-fg-muted group-disabled:text-ui-fg-disabled text-sm">{hint}</Label>}
      </button>
      <input hidden ref={inputRef} onChange={handleFileChange} type="file" accept={formats.join(",")} multiple={multiple} />
    </div>
  )
}
